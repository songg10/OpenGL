...
/* This implimentation of FruitTetris is studied from the implimentation of
   Ivan Jonathan Hoo (301251368) with modifications and additional features */
...
//occupied coordinate array represents coordinate currently occupied by tile
vec2 occupiedCoor[4];
...
int tileType; //1 represents "I" tile, 2 represents "S" tile, 3 represents "L" tile, "4" represent "T" tile
...
void newtile()
{
	...
	tileType = rand() % 4 + 1; //select tile randomly (either I, S, or L)
	...
	int boxColorInt = -10; //-1 represents black, 0 represents purple, 1 represents red, 2 represents yellow, 3 represents green, 4 represents orange , -10 is for initialization
	int selectedIndex = -10; //-10 is for initialization
	vector<int> colorIntVector; // C++ vector data structure
	for(int i=0;i<5;i++)
...
void reverseTileColour() //for "I" and "S" tile rotational purpose only
{
	//-1 represents black, 0 represents purple, 1 represents red, 2 represents yellow, 3 represents green, 4 represents orange
	int aColor=currentTileColor[0];
	int bColor=currentTileColor[1];
...
void rotate()
{
	...
	else //"T" tile
	{
		...
		for (int i = 0; i < 4; i++) //check if tile can rotate
		{
			int XX = tilepos.x + allRotationsTshape[rotationIndex][i].x;
			int YY = tilepos.y + allRotationsTshape[rotationIndex][i].y;
			if(XX<=-1 || XX>9 || YY<0 || YY>19 || board[XX][YY]==true)
			{
				rotationIndex--;
				return;
		...
		for (int i = 0; i < 4; i++)
			tile[i] = allRotationsTshape[rotationIndex][i]; // Get the 4 pieces of the new tile
	}
	updatetile();
}
...
void checkSupport(int column) //check for tiles with no support
{
	int indexRow = -1;
	...
	int different=indexRow-k;

	//shift a coln down different rows
	int b=k+1;
	while ( (b+different-1)<20 )
	{
		board[column][b]=board[column][b+different-1];
		cellsColours[column][b]=cellsColours[column][b+different-1];
		b++;
	}
	for(int c=b; c<20; c++)
	{
...
{
	glutTimerFunc(SPEED, fallingTileAuto, -1);
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("Song's Fruit Tetris");
	glewInit();
	init();
